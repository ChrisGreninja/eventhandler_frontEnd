...
function Event() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch event details
                const eventRes = await axios.get(`http://localhost:8081/events/${id}`, { withCredentials: true });
    ...
    const handleShowAttendees = async () => {
        try {
            const res = await axios.get(`http://localhost:8081/events/${id}/attendees`, { withCredentials: true });
            if (res.data.Error) {
