...
function EventPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch events
                const eventsRes = await axios.get('http://localhost:8081/events', { withCredentials: true });
                setEvents(eventsRes.data || []);
                
                // For each event, fetch its attendee count
                for (const event of eventsRes.data) {
                    try {
                        const eventRes = await axios.get(`http://localhost:8081/events/${event._id}`, { withCredentials: true });
                ...
                // Fetch user data
                const userRes = await axios.get('http://localhost:8081/user', { withCredentials: true });
    ...
    const handleEventSubmit = async (e) => {
        ...
        try {
            const res = await axios.post('http://localhost:8081/events/create', newEvent, { withCredentials: true });
            if (res.data.Status === 'Event created successfully!') {
                alert('Event Created Successfully');
                // Refresh events list after creation
                try {
                    const response = await axios.get('http://localhost:8081/events', { withCredentials: true });
    ...
    const handleJoinEvent = async (eventId) => {
        ...
        try {
            const response = await axios.post('http://localhost:8081/events/join', 
                { eventId },
